...
import { analyzeBookshelf } from '../../services/ai';
import { supabase } from '../../lib/supabase';

export default function ShelfCameraScreen() {
  ...
  if (!permission.granted) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background, justifyContent: 'center', alignItems: 'center', padding: 20 }]}>
        <Text style={[styles.message, { color: colors.text }]}>Camera access is required to scan your bookshelf.</Text>
  ...
  const processImage = async (base64: string) => {
    try {
      ...
      const analysis = await analyzeBookshelf(base64, genre, grade);

      // Save to database
      const { data, error } = await supabase.from('book_scans').insert({
        user_id: user.id,
        title: 'Bookshelf Scan',
        author: 'Multiple Authors',
        genre: genre,
        reading_level: grade,
        maturity_level: 'Variable',
        is_movie: false,
      ...
      // Ensure we navigate to the new shelf-result screen with the ID so it knows to render it nicely
      router.replace({ pathname: '/scan/shelf-result', params: { id: data[0].id } });
    } catch (error: any) {
      Alert.alert('Shelf Analysis Failed', error.message || 'Something went wrong');
    } finally {
      setLoading(false);
    }
  };
      ...
      <CameraView style={styles.camera} ref={cameraRef} flash={flash}>
        <View style={styles.overlay}>
          ...
          <View style={styles.guideContainer}>
            <Text style={styles.guideText}>Point camera at the entire bookshelf</Text>
          </View>
